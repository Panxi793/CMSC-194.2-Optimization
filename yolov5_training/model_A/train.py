...
def find_datasets():
    ...
    dataset_root = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..', 'datasets', 'drone_dataset'))
    dataset_dirs = glob.glob(os.path.join(dataset_root, 'dataset*'))
    
    if not dataset_dirs:
        raise FileNotFoundError(f"No dataset directories found in {dataset_root}")
...
def create_dataset_yaml():
    ...
    # Split datasets - use all for training and validation
    train_dirs = dataset_dirs
    val_dirs = dataset_dirs  # Same as training for this example
    ...
    # Populate train and val with image paths
    for d in train_dirs:
...
def main():
    print("=== YOLOv5 Training for Model A (Original VisDrone2019) ===")
    ...
    print("Training completed for Model A.")
